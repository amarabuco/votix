...
timer = pd.to_datetime('2024-10-06 08:00:00') - datetime.today()
days = (timer.days)
...
st.write(f"👉 <a target='_blank' href='https://tinyurl.com/votix-br/ranking'> RANKING</a>",
...
st.write(f""" ## {municipio} | {cargo}""")

# f"https://divulgacandcontas.tse.jus.br/divulga/rest/v1/candidatura/listar/2024/{sigla}/2045202024/{cargo_id}/candidatos"
# r = requests.get(f"https://divulgacandcontas.tse.jus.br/divulga/rest/v1/candidatura/listar/2024/{sigla}/2045202024/{cargo_id}/candidatos", headers=headers)
# st.write(r)
...
cid = candidatos_partido.loc[candidatos_partido['nomeUrna']
                             == nome].values[0, 0]
...
props = ["grauInstrucao", "descricaoTotalizacao",
         "eleicoesAnteriores", "dataDeNascimento"]
candidato_resumo = pd.Series(
    {p: candidato[p] for p in props}, name=candidato['id'])
...
with st.container():
    st.image(
        f"https://divulgacandcontas.tse.jus.br/divulga/images/partidos/{partido}.jpg", width=128)
    partido
...
with col1:
    ...
    tse = f"https://divulgacandcontas.tse.jus.br/divulga/#/candidato/2024/2045202024/{sigla}/{cid}"
    ...
    # st.write(tse)
    st.write(
        f"👉 <a target='_blank' href='{tse}'> TSE</a>", unsafe_allow_html=True)
    st.write(
        f"👉 <a target='_blank' href='{wk}'> WIKIPEDIA</a>", unsafe_allow_html=True)
    st.write(
        f"👉 <a target='_blank' href='{news}'> NOTÍCIAS</a>", unsafe_allow_html=True)
with col2:
    st.write('#### 📋 Dados ')
    ...
    st.write('🌎 Naturalidade:',
             candidato['nomeMunicipioNascimento'], '-', candidato['sgUfNascimento'])
    st.write('📚 Escolaridade:',
             f"{candidato['grauInstrucao']} <a target='_blank' href='{lattes}'> LATTES</a>", unsafe_allow_html=True)
...
score = pd.Series(get_score(candidato_resumo)).reset_index().rename(
    {'index': 'critério', 0: 'pontos'}, axis=1)
st.write(pd.concat([score, pd.DataFrame(
    {'critério': 'média', 'pontos': score['pontos'].mean()}, index=[len(score)])]))
...
fig = px.line_polar(score,  r='pontos', theta='critério',
...
st.write(eleicoes[['nrAno', 'nomeUrna', 'partido',
         'cargo', 'situacaoTotalizacao']])

st.info("### Despesas")
try:
    st.metric('Total Recebido', '{:,.2f}'.format(
        prestacao['dadosConsolidados']['totalRecebido']))
except:
    pass
try:
    st.metric('Despesas Contratadas', '{:,.2f}'.format(
        prestacao['despesas']['totalDespesasContratadas']))
except:
    pass
st.warning("#### Doadores")
