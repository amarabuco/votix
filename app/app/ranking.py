...
candidatos_df = pd.DataFrame.from_records(requests.get(
...
vagas = pd.read_csv(
    f'https://raw.githubusercontent.com/amarabuco/votix/data/app/app/data/candidatos/consulta_vagas_2022/consulta_vagas_2022_{sigla}.csv', sep=';', encoding='latin1')

vg = vagas.query(f'cargo == {cargo_id}')['QT_VAGAS'].values[0]
st.warning(
...
eleicoes_anteriores = resultado.merge(eleicoes_anteriores, on='NR_CPF_CANDIDATO').query(
    f"DS_SIT_TOT_TURNO == 'ELEITO' or DS_SIT_TOT_TURNO == 'ELEITO POR QP'")
eleicoes_anteriores = eleicoes_anteriores[[
    'NR_CPF_CANDIDATO', 'NM_URNA_CANDIDATO_x', 'ANO', 'SG_PARTIDO_y', 'DS_CARGO', 'NM_UE']]
...
resultado['escolaridade'] = resultado['DS_GRAU_INSTRUCAO'].apply(
    get_escolaridade)
resultado['anos'] = ((datetime.now() - pd.to_datetime(resultado['DT_NASCIMENTO'],
                     format='%d/%m/%Y')).dt.days/365.2425).astype('int')
...
resultado['politica'] = resultado['NR_CPF_CANDIDATO'].apply(
...
resultado['posicao'] = resultado['SG_PARTIDO'].apply(
...
cols = ['NM_URNA_CANDIDATO', 'NR_CANDIDATO', 'SG_PARTIDO', 'posicao', 'ST_REELEICAO', 'ranking',
        'escolaridade', 'idade', 'politica', 'anos', 'DS_GENERO', 'DS_COR_RACA', 'DS_GRAU_INSTRUCAO', 'DS_OCUPACAO', ]
...
fig = px.scatter(resultado,  x='ranking', y='anos', color='ST_REELEICAO', hover_data=[
                 'NM_URNA_CANDIDATO', 'NR_CANDIDATO', 'SG_PARTIDO', 'DS_GRAU_INSTRUCAO', 'DS_GENERO', 'DS_COR_RACA', 'DS_OCUPACAO'])
st.plotly_chart(fig, use_container_width=True)

st.info('Cargos Eletivos')
st.dataframe(cargos_eletivos[['NM_URNA_CANDIDATO_x', 'ANO',
             'SG_PARTIDO_y', 'DS_CARGO', 'NM_UE', 'politica']])

st.info('Bens declarados')
resultado_bens = candidatos_df.merge(
    bens, on='SQ_CANDIDATO').sort_values('NM_URNA_CANDIDATO')
...
resultado_bens = resultado_bens.loc[resultado_bens['NR_CANDIDATO'].isin(
    resultado['NR_CANDIDATO'])]
st.write(resultado_bens[['NM_URNA_CANDIDATO', 'NR_CANDIDATO',
         'SG_PARTIDO', '2010', '2014', '2018', '2022', 'DIFERENCA']])
