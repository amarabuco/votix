...
def get_score(df):
    ...
    idade = (
        (datetime.now() - pd.to_datetime(df['dataDeNascimento']))/365.2425).days
    score_idade = 0
    ...
    cargos = {'Nenhum': 0, '1º Suplente': 0, '2º Suplente': 0,  '1º Suplente Senador': 0, '2º Suplente Senador': 0, 'Vereador': 1, 'Deputado Estadual': 2, 'Deputado Federal': 3,
              'Vice-prefeito': 2.5, 'Vice-Prefeito': 2.5, 'Prefeito': 3, 'Senador': 3.5, 'Vice-governador': 4, 'Vice-Governador': 4, 'Governador': 4.5, 'Vice-presidente': 4.5, 'Presidente': 5}
    eleicoes = pd.DataFrame(df['eleicoesAnteriores'])
    eleicoes = eleicoes.query(
        f"situacaoTotalizacao == 'Eleito' or situacaoTotalizacao == 'Eleito por QP'")
...
def get_politica(x):
    cargos = {'nenhum': 0, '1º suplente': 0, '2º suplente': 0,  '1º suplente Senador': 0, '2º suplente Senador': 0, 'vereador': 1, 'deputado estadual': 2,
              'deputado federal': 3, 'vice-prefeito': 2.5, 'prefeito': 3, 'senador': 3.5, 'vice-governador': 4, 'governador': 4.5, 'vice-presidente': 4.5, 'presidente': 5}
...
def get_eleicoes():
    ...
    eleitos2022 = pd.read_csv(f'https://raw.githubusercontent.com/amarabuco/votix/data/app/app/data/candidatos/consulta_cand_2022/eleitos_2022.csv',
                              sep=',',  converters={'NR_CPF_CANDIDATO': lambda x: str(x)})
    eleitos2022['ANO'] = 2022

    eleicoes = [eleitos2010, eleitos2012, eleitos2014,
                eleitos2016, eleitos2018, eleitos2020, eleitos2022]
    return eleicoes
